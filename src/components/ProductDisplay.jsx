...
const ProductDisplay = (props) => {
    ...
    const currentProduct = all_products.find((p) => p.id === product.id);
            ...
            <div className='flex flex-col gap-14 xl:flex-row'>
                <div className='flex gap-x-2 xl:flex-[1]'>
                    <div className='flex flex-col gap-[7px] flex-wrap'>
                        {/* Display small images for the current product */}
                        {currentProduct && currentProduct.small_images && currentProduct.small_images.map((img, index) => (
                            <img key={index} src={img[0]} alt={`Small Image ${index + 1}`} />
                        ))}
                    </div>
                    <div>
